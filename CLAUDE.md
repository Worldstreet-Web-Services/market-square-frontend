...
- **Purple is one shared ramp with two named stops carrying distinct roles. Keep the tokens separate; do not add a third purple.** The stops are genuinely related — the Join Live Arena CTA proves it by running a gradient straight from one into the other — so do not describe them as unrelated hues. What must not collapse is the *semantics*, not the palette
  - `--color-create` `#9f65fd` → `--color-create-deep` `#5b05e6`, white label, utility `ws-btn-create`. Role: **the compose CTA**. The 2026-08-25 revision repainted Create Post off the amber ramp onto this
  - `--color-spotlight` `#7e3beb` (card wash + hairline), `--color-spotlight-chip` `#6829ce`, `--color-spotlight-chip-ink` `#c27aff`, utility `ws-spotlight`. Role: **featured citizens**. A later revision repainted the Citizen Spotlight surface off the amber ramp onto this — the rail card, its FEATURED chip, and `/spotlight`'s window chip, rank-1 podium badge and top-3 rank numerals. Its heading is now plain **white**, not amber
  - **Both together** = the featured hero's **live-arena CTA**, `ws-btn-arena`: `linear-gradient(131.93deg, --color-spotlight -10.87%, --color-spotlight 43.15%, --color-create 109.17%)`. It reuses the two existing tokens rather than defining its own purple — any new purple surface should do the same
  - They stay separate tokens so a repaint of one role cannot silently drag the other, and so this gradient keeps naming its endpoints. Do not merge them into a single token, and do not re-read the old "NO purple" rule as covering any of them
...
- `--color-featured` (amber) is semantic — **featured, premium, top-ranked, or coin value**, never decoration. The ramp is `--color-featured` `#e8b74a` → `--color-featured-deep` `#cda243`, `--color-featured-hi` `#ffb900` and `--color-featured-chip` `#ffd230` are the leftovers of Citizen Spotlight's old amber heading and chip and are now **unused** — do not reach for them. Still in use: the verification card's `ws-featured` wash, VIP ticket tiers, the paid Supporter badge, unread count badges, and every coin mark in the live room. **Not** the liked heart (red since 2026-08-25), **not** Create Post (violet since the same revision) and **not** Citizen Spotlight or `/spotlight` (purple since the Spotlight revision) — all three were amber and all three moved off it, so treat "it used to be amber" as no argument for making something amber. `Pill` carries a `featured` tone for it. Everything else stays on the silver ramp
