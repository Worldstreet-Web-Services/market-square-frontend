...
- **There are exactly two purples and they are different hues with different meanings. Never consolidate them into one token.**
  - `--color-create` `#9f65fd` → `--color-create-deep` `#5b05e6`, white label, utility `ws-btn-create`. Meaning: **the primary action**. The 2026-08-25 revision repainted the Create Post CTA off the amber ramp onto this. It is the CTA hue and nothing else
  - `--color-spotlight` `#7e3beb` (card wash + hairline), `--color-spotlight-chip` `#6829ce`, `--color-spotlight-chip-ink` `#c27aff`, utility `ws-spotlight`. Meaning: **this citizen is featured**. A later revision repainted the Citizen Spotlight surface off the amber ramp onto this — the rail card, its FEATURED chip, and `/spotlight`'s window chip, rank-1 podium badge and top-3 rank numerals. Its heading is now plain **white**, not amber
  - They are separate tokens so that repainting the CTA can never drag the Spotlight card with it, and vice versa. Anyone tempted to "unify the violets" is deleting a semantic distinction, not removing duplication. Do not generalise either into a violet ramp for other surfaces, and do not re-read the old "NO purple" rule as covering them
