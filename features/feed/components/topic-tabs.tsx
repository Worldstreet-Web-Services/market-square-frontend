"use client";

import { cn } from "@/lib/cn";

/**
 * HOME'S TAB ROW — node 647:16266 in the live file (updated 2026-09-10); first
 * built from 225:3352, whose pill the live node keeps.
 *
 * `For you · Tech · Entertainment · Crypto & Web3 · Science · Real Estate ·
 * Music · Relationships · Gaming`.
 *
 * ─── WHAT THIS REPLACES, AND WHY IT MATTERS ──────────────────────────────────
 * These are TOPICS, not lanes. Home used to head the timeline with
 * `For You · Following · Trending` — three ways of ranking the same posts,
 * which tells a reader nothing about what the square is talking about. The file
 * heads it with the subjects instead, which is the whole proposition of the
 * product: you come here to find the conversation you care about.
 *
 * `For you` is the unfiltered lane. Every other pill is a key from the SHARED
 * VOCABULARY served by `GET /topics` — the same keys that tag a post, express a
 * reader's interests and filter a gist room. Nothing here is hard-coded: the
 * row renders what the service serves, in the backend's own `sortOrder`, so a
 * topic added upstream appears with no client change. The names in the file are
 * that vocabulary's own labels.
 *
 * ─── THE FILE'S NUMBERS ──────────────────────────────────────────────────────
 * Each pill is 101x38 at a full round, 10px of padding, the label at Bold 12/16.
 * The SELECTED pill carries `linear-gradient(226deg, #7E3BEB 22.4%, #472185
 * 84.9%)` with `#F4F4F4` ink; the rest are transparent with 40% white. The
 * angle and stops are the file's handles projected onto the real 101 x 38 box.
 * The MCP summary reports "201deg 13% -> 100%", which is the angle in the unit
 * square; on a pill that wide it drew the left end visibly lighter than the
 * file's render, and 226deg matches the render at both ends. Under the row,
 * a 2px `white/08` rule runs the full width.
 *
 * Both stops are the FILE'S, not a token substitution. The light one happens to
 * be `--color-spotlight`; the dark one, `#472185`, has no token and is written
 * literally rather than rounded to the nearest purple the palette already owns.
 * A gradient's far stop is not a surface colour and does not need naming — what
 * it needs is to be right.
 *
 * ─── THE "FOR YOU" GLYPH ─────────────────────────────────────────────────────
 * The live node puts the file's wink (677:18745, exported, a 19.5 glyph in a 24
 * box) 3px before "For you", tinted #D8BCFF on the selected gradient. It
 * belongs to the For you pill, not to selection: it is a child of that pill in
 * the file. Unselected, it takes the label's 40% white so the row still reads
 * as one weight. The label is Geist, not the file's Roboto, as everywhere else.
 *
 * The row sits on the page's own ground (`--color-chrome`, `#121214` at full
 * opacity) with 5px between it and the rule, so it is invisible as a band.
 *
 * A pill is 101 wide only when its label fits — "Crypto & Web3" and
 * "Relationships" do not, and the file lets those grow. So the width is a
 * MINIMUM here rather than a fixed size, which is what the file draws once you
 * look at the wide pills.
 *
 * The row SCROLLS: it is 908 wide inside an 805 column in the file, so it is
 * over-wide by design and the last topics are reached by dragging. The
 * scrollbar is hidden, as it is on every other rail in the app.
 */
export interface TopicTab {
  /** The vocabulary key, or null for "For you" — the unfiltered lane. */
  key: string | null;
  label: string;
}

export function TopicTabs({
  tabs,
  active,
  onSelect,
}: {
  tabs: readonly TopicTab[];
  /** The selected key; null is "For you". */
  active: string | null;
  onSelect: (key: string | null) => void;
}) {
  return (
    <div className="flex flex-col gap-[5px] bg-chrome">
      <div
        aria-label="Topics"
        className="flex items-center gap-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        {tabs.map((tab) => {
          const on = tab.key === active;
          return (
            <button
              key={tab.key ?? "for-you"}
              type="button"
              onClick={() => onSelect(tab.key)}
              aria-current={on ? "true" : undefined}
              className={cn(
                "ws-press flex h-[38px] min-w-[101px] shrink-0 items-center justify-center gap-[3px] rounded-full px-2.5 text-[12px] font-bold leading-4 transition-colors",
                on
                  ? "bg-[linear-gradient(226deg,#7E3BEB_22.4%,#472185_84.9%)] text-grey-100"
                  : "text-white/40 hover:text-body"
              )}
            >
              {tab.key === null && (
                <IconForYou className={cn("h-6 w-6 shrink-0", on && "text-[#D8BCFF]")} />
              )}
              {tab.label}
            </button>
          );
        })}
      </div>
      {/* The file's own 2px rule, full width, at 8% white. */}
      <span aria-hidden className="h-0.5 w-full bg-white/[0.08]" />
    </div>
  );
}

/**
 * The "For you" pill's wink — node 677:18745 in 647:16266, exported from the
 * file. The file fills it #D8BCFF; `currentColor` here so the pill sets it.
 */
function IconForYou({ className }: { className?: string }) {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12 2.25C10.0716 2.25 8.18657 2.82183 6.58319 3.89317C4.97982 4.96451 3.73013 6.48726 2.99218 8.26884C2.25422 10.0504 2.06114 12.0108 2.43735 13.9021C2.81355 15.7934 3.74215 17.5307 5.10571 18.8943C6.46928 20.2579 8.20656 21.1865 10.0979 21.5627C11.9892 21.9389 13.9496 21.7458 15.7312 21.0078C17.5127 20.2699 19.0355 19.0202 20.1068 17.4168C21.1782 15.8134 21.75 13.9284 21.75 12C21.7473 9.41498 20.7192 6.93661 18.8913 5.10872C17.0634 3.28084 14.585 2.25273 12 2.25ZM12 20.25C10.3683 20.25 8.77326 19.7661 7.41655 18.8596C6.05984 17.9531 5.00242 16.6646 4.378 15.1571C3.75358 13.6496 3.5902 11.9908 3.90853 10.3905C4.22685 8.79016 5.01259 7.32015 6.16637 6.16637C7.32016 5.01259 8.79017 4.22685 10.3905 3.90852C11.9909 3.59019 13.6497 3.75357 15.1571 4.37799C16.6646 5.00242 17.9531 6.05984 18.8596 7.41655C19.7661 8.77325 20.25 10.3683 20.25 12C20.2475 14.1873 19.3775 16.2843 17.8309 17.8309C16.2843 19.3775 14.1873 20.2475 12 20.25ZM7.5 10.125C7.5 9.9025 7.56598 9.68499 7.6896 9.49998C7.81322 9.31498 7.98892 9.17078 8.19449 9.08564C8.40005 9.00049 8.62625 8.97821 8.84448 9.02162C9.06271 9.06502 9.26317 9.17217 9.4205 9.3295C9.57783 9.48684 9.68498 9.68729 9.72839 9.90552C9.7718 10.1238 9.74952 10.35 9.66437 10.5555C9.57922 10.7611 9.43503 10.9368 9.25002 11.0604C9.06502 11.184 8.84751 11.25 8.625 11.25C8.32664 11.25 8.04049 11.1315 7.82951 10.9205C7.61853 10.7095 7.5 10.4234 7.5 10.125ZM17.25 10.125C17.25 10.3239 17.171 10.5147 17.0303 10.6553C16.8897 10.796 16.6989 10.875 16.5 10.875H14.25C14.0511 10.875 13.8603 10.796 13.7197 10.6553C13.579 10.5147 13.5 10.3239 13.5 10.125C13.5 9.92609 13.579 9.73532 13.7197 9.59467C13.8603 9.45402 14.0511 9.375 14.25 9.375H16.5C16.6989 9.375 16.8897 9.45402 17.0303 9.59467C17.171 9.73532 17.25 9.92609 17.25 10.125ZM16.3988 14.625C15.4341 16.2928 13.8309 17.25 12 17.25C10.1691 17.25 8.56594 16.2937 7.60032 14.625C7.55107 14.5397 7.51911 14.4455 7.50626 14.3478C7.49342 14.2502 7.49993 14.1509 7.52544 14.0558C7.55095 13.9606 7.59495 13.8714 7.65493 13.7933C7.7149 13.7151 7.78969 13.6496 7.875 13.6003C7.96032 13.5511 8.0545 13.5191 8.15217 13.5063C8.24984 13.4934 8.34909 13.4999 8.44424 13.5254C8.53939 13.5509 8.62858 13.5949 8.70673 13.6549C8.78488 13.7149 8.85045 13.7897 8.89969 13.875C9.59907 15.0853 10.6997 15.75 12 15.75C13.3003 15.75 14.4009 15.0844 15.1013 13.875C15.1481 13.7854 15.2127 13.7062 15.291 13.6421C15.3693 13.5781 15.4597 13.5305 15.5569 13.5023C15.654 13.4742 15.7558 13.4659 15.8563 13.4781C15.9567 13.4903 16.0536 13.5227 16.1412 13.5733C16.2287 13.6239 16.3052 13.6917 16.3658 13.7727C16.4265 13.8536 16.4702 13.946 16.4943 14.0442C16.5183 14.1425 16.5223 14.2446 16.5059 14.3444C16.4895 14.4442 16.453 14.5396 16.3988 14.625Z"
        fill="currentColor"
      />
    </svg>
  );
}
