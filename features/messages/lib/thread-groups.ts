...
export interface SenderRun<T> {
  /** The first message's id — stable across a poll tick, unlike an index. */
  key: string;
  senderId: string;
  messages: T[];
}
...
/**
 * Split a day's messages into consecutive same-sender RUNS.
 *
 * Both nodes stack a day's messages at `gap: 16` inside a `gap: 24` day block,
 * so the river's rhythm is TWO numbers, not one: tight inside a run, loose
 * between runs and under the separator. That is what makes a burst of three
 * messages read as one turn in the conversation rather than as three events,
 * and it cannot be expressed by a single `gap` on the day.
 *
 * Consecutive, NOT bucketed by sender: two separate things Fatima said either
 * side of a reply are two runs, and collapsing them would reorder the
 * conversation. Same discipline as `groupMessagesByDay`, which groups
 * consecutive days for the same reason.
 *
 * An empty `senderId` (the schema's default for a payload that omits it) is
 * its own run key like any other, so a service that stops sending sender ids
 * degrades to one long run rather than throwing.
 */
export function groupBySender<T extends Pick<Message, "id" | "senderId">>(
  messages: T[]
): Array<SenderRun<T>> {
  const runs: Array<SenderRun<T>> = [];
  ...
  for (const message of messages) {
    ...
    runs.push({ key: message.id, senderId: message.senderId, messages: [message] });
  }
