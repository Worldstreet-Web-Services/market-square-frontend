...
export function BuyGiftSheet({
  ...
  /**
   * A purchase is in flight. The action says so and refuses a second press:
   * this charges money, and a double tap that creates two purchases is the
   * one mistake an idempotency key cannot fix from the UI side — it would be
   * two distinct intents, not one retried.
   */
  busy?: boolean;
}) {
  return (
    ...
    <>
      ...
      <div className="mt-2 flex flex-col gap-6">
        ...
        <button
          ...
          disabled={Boolean(disabledReason) || !onConfirm || busy}
