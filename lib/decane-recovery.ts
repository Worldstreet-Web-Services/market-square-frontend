...
// Bridges Decane's wallet-recovery callbacks to React UI (ported from wsws). The callbacks are
...
// request here and awaits it, and DecaneRecoveryHost (components/providers) renders the matching
...
// kit's storage layout on purpose (see hooks/use-auth); a wrong guess
 ...
 * (`set`) and to enter it (`unlock`). Kept distinct from promptPin: this one can
 * open a session by itself, so it deserves different wording.
 */
...
/**
 * Decane's `promptPin`. Called when this device has no usable passkey, either to
 * choose the PIN that wraps the device share or to re-enter it on unlock. The
 * kit gives no argument distinguishing the two, so the dialog infers it and
