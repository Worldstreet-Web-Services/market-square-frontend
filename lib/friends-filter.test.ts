...
test("the resting filter narrows nothing, and still leaves out people the reader follows", () => {
  ...
  // never held them.
  assert.deepEqual(friendsFilterFacets(EMPTY_FRIENDS_FILTER), { excludeFollowing: true, excludeWinked: true, excludeWinkedEver: true, excludePassed: true });
});
...
test("one clause names its value on the pill and sends only that facet", () => {
  ...
  assert.deepEqual(friendsFilterFacets(city), { city: "Lagos", excludeFollowing: true, excludeWinked: true, excludeWinkedEver: true, excludePassed: true });

  const gender = { ...EMPTY_FRIENDS_FILTER, gender: "Female" };
  assert.equal(friendsFilterLabel(gender), "Female");
  assert.deepEqual(friendsFilterFacets(gender), { gender: "Female", excludeFollowing: true, excludeWinked: true, excludeWinkedEver: true, excludePassed: true });
  ...
  assert.deepEqual(friendsFilterFacets(everyone), { excludeWinked: true, excludeWinkedEver: true, excludePassed: true });
});
...
test("whitespace is not a clause", () => {
  ...
  assert.deepEqual(friendsFilterFacets(blank), { excludeFollowing: true, excludeWinked: true, excludeWinkedEver: true, excludePassed: true });
});
