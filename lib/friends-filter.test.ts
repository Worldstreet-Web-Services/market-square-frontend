...
test("the resting filter narrows nothing, and still leaves out people the reader follows", () => {
  ...
  // never held them.
  assert.deepEqual(friendsFilterFacets(EMPTY_FRIENDS_FILTER), { excludeFollowing: true });
});
...
test("one clause names its value on the pill and sends only that facet", () => {
  ...
  assert.deepEqual(friendsFilterFacets(city), { city: "Lagos", excludeFollowing: true });

  const gender = { ...EMPTY_FRIENDS_FILTER, gender: "Female" };
  assert.equal(friendsFilterLabel(gender), "Female");
  assert.deepEqual(friendsFilterFacets(gender), { gender: "Female", excludeFollowing: true });
  ...
  assert.deepEqual(friendsFilterFacets(everyone), {});
});
...
test("whitespace is not a clause", () => {
  ...
  assert.deepEqual(friendsFilterFacets(blank), { excludeFollowing: true });
});
